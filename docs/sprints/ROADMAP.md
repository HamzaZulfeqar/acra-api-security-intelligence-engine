...
- Phase 3 run: `36123760660` — SUCCESS at `be10c1c406bb475e12446ad9cff23056c3cc352a`;
...
- Burp AuditIssue adapter: VERIFIED;
...
Verified Phase 3:
- Montoya-only AuditIssue adapter: PASS;
- CANDIDATE/issueEligible gate: PASS;
- real request/response + URL requirement: PASS;
- conservative severity/confidence mapping: PASS;
- review-only / human-validation issue wording: PASS;
- dynamic HTML escaping / secret exclusion: PASS;
- Burp adapter suite: PASS, 32 assertions;
- retained Sprint 2/3 offline Montoya compatibility: PASS.

Next dependency-ordered milestone:
1. add explicit Site Map publisher using `SiteMap.add(AuditIssue)`;
2. deduplicate on stable reproduction fingerprint before publication;
3. make publication opt-in/explicit rather than passive side effect;
4. verify publisher behavior with a headless Montoya contract stub;
5. keep real desktop acceptance/display separately UNVERIFIED.
