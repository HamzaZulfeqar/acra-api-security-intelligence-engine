...
cd "$ROOT"
OUT="$ROOT/build/s10-final-package"
rm -rf "$OUT"
mkdir -p "$OUT"
...
root=Path.cwd()
out=root/"build"/"s10-final-package"
zip_path=out/"acra-sprint-10-final.zip"
manifest_path=out/"sprint10-package-manifest.txt"
verify_path=out/"sprint10-package-verification.json"
sha_path=out/"acra-sprint-10-final.zip.sha256"

excluded_parts={".git","build","dist","target","__pycache__",".idea",".vscode",".acra"}
...
with zipfile.ZipFile(zip_path) as archive:
    ...
    with tempfile.TemporaryDirectory(prefix="acra-s10-verify-") as temp:
        target=Path(temp)
...
verification={
    ...
    "excludedBuildDistTargetAndGitArtifacts":True,
}
verify_path.write_text(json.dumps(verification,indent=2,sort_keys=True)+"\n",encoding="utf-8")
print(json.dumps(verification,sort_keys=True))
PY

echo "SPRINT10_PACKAGE_VERIFICATION PASS"
