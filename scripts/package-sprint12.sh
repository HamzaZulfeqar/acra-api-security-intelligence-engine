...
cd "$ROOT"
OUT="$ROOT/build/s12-final-package"
rm -rf "$OUT"
mkdir -p "$OUT"
...
root=Path.cwd()
out=root/"build"/"s12-final-package"
zip_path=out/"acra-sprint-12-final.zip"
manifest_path=out/"sprint12-package-manifest.txt"
verify_path=out/"sprint12-package-verification.json"
sha_path=out/"acra-sprint-12-final.zip.sha256"
...
with zipfile.ZipFile(zip_path) as archive:
    ...
    with tempfile.TemporaryDirectory(prefix="acra-s12-verify-") as temp:
        target=Path(temp)
...
verification={
    ...
    "interoperabilityScope":"FR-013 JSON/SARIF/Burp Issue software interoperability",
    "jsonCapability":"IMPLEMENTED",
    "sarifCapability":"IMPLEMENTED",
    "burpIssueCapability":"IMPLEMENTED_RUNTIME_UNVERIFIED",
    "realBurpDesktopRuntime":"UNVERIFIED_DEFERRED",
}
verify_path.write_text(json.dumps(verification,indent=2,sort_keys=True)+"\n",encoding="utf-8")
print(json.dumps(verification,sort_keys=True))
PY

echo "SPRINT12_PACKAGE_VERIFICATION PASS"
