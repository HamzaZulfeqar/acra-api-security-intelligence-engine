...
cd "$ROOT"
OUT="$ROOT/build/s8-final-package"
rm -rf "$OUT"
mkdir -p "$OUT"
...
root=Path.cwd()
out=root/"build"/"s8-final-package"
zip_path=out/"acra-sprint-08-final.zip"
manifest_path=out/"sprint8-package-manifest.txt"
verify_path=out/"sprint8-package-verification.json"
sha_path=out/"acra-sprint-08-final.zip.sha256"
...
with zipfile.ZipFile(zip_path) as archive:
    ...
    with tempfile.TemporaryDirectory(prefix="acra-s8-verify-") as temp:
        target=Path(temp)
...
echo "SPRINT8_PACKAGE_VERIFICATION PASS"
