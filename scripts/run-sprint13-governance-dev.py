...
REGISTRY=ROOT/"lab/ground-truth/POL-S13-GOV-DEV-001.json"
...
A7=load(S12,"a7_gov_dev"); D=load(DIM,"dim_gov_dev"); P=load(POL,"pol_gov_dev"); G=load(GOV,"gov_dev")
...
def main():
    ...
    cases=data["cases"]
    if len(cases)!=64: raise AssertionError("expected 64 dev cases")
    rows=[]
    for c in sorted(cases,key=lambda x:x["caseId"]):
        ...
        old_g1=P.apply_policy_to_prediction(locked,policy)
        ...
        rows.append({
            ...
            "lockedA7Prediction":locked["prediction"],"legacyG1Prediction":old_g1["prediction"],
    ...
    artifact={"schemaVersion":"s13-gov-dev-predictions-v1","sourceCommit":head(),"featureDatasetId":data["datasetId"],
      "featureDatasetSha256":sha(FEATURES),"policyRegistrySha256":sha(REGISTRY),"governanceEngineSha256":sha(GOV),
      "lockedPolicyEngineSha256":sha(POL),"lockedDimensionEngineSha256":sha(DIM),"lockedA7Sha256":sha(S12),
      "caseCount":len(rows),"boundary":{"labelsAvailableToPredictor":False,"registeredDimensionAvailableToPredictor":False}}
    ...
    print(f"SPRINT13_GOV_DEV_PREDICTION PASS cases={len(rows)}")
