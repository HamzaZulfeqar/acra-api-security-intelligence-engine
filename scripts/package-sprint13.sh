...
cd "$ROOT"
OUT="$ROOT/build/s13-final-package"
rm -rf "$OUT"
mkdir -p "$OUT"
...
root=Path.cwd()
out=root/"build"/"s13-final-package"
zip_path=out/"acra-sprint-13-final.zip"
manifest_path=out/"sprint13-package-manifest.txt"
verify_path=out/"sprint13-package-verification.json"
sha_path=out/"acra-sprint-13-final.zip.sha256"
...
with zipfile.ZipFile(zip_path) as archive:
    ...
    with tempfile.TemporaryDirectory(prefix="acra-s13-verify-") as temp:
        target=Path(temp)
...
verification={
    ...
    "findingCandidateAutoConfirmation":"FORBIDDEN",
    "governanceInitialState":"REVIEW_REQUIRED",
    "realBurpDesktopRuntimePublication":"UNVERIFIED_DEFERRED",
}
verify_path.write_text(json.dumps(verification,indent=2,sort_keys=True)+"\n",encoding="utf-8")
print(json.dumps(verification,sort_keys=True))
PY

echo "SPRINT13_PACKAGE_VERIFICATION PASS"
