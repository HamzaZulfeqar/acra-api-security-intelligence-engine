#!/usr/bin/env python3
"""Phase 3 untouched evaluation prediction: frozen locked A7 vs frozen policy-generalized G1."""
...
OUT_DIR=ROOT/"build/s13-policy-eval"
...
S12=load(S12_RUNNER,"s12_policy_eval")
DIM=load(DIM_MODULE,"dim_policy_eval")
POL=load(POLICY_MODULE,"policy_eval")
...
def main():
    ...
    if data.get("datasetId")!="GT-S13-POLICY-EVAL-FEATURES": raise AssertionError("unexpected eval corpus")
    if data.get("createdAfterAlgorithmFreeze")!="c7c66336daf050753aa0ac4fb3a1f293c5d1e2dd": raise AssertionError("algorithm-freeze provenance drift")
    if data.get("labelSeparated") is not True: raise AssertionError("evaluation labels must be separated")
    if data.get("policyRegistryId")!=registry.get("registryId"): raise AssertionError("policy registry identity mismatch")
    cases=data.get("cases")
    if not isinstance(cases,list) or len(cases)!=24: raise AssertionError("expected 24 untouched evaluation cases")
    rows=[]
    for case in sorted(cases,key=lambda x:x["caseId"]):
        if any(k in case for k in ("dimension","groundTruth","secureExpected","vulnerableExpected","expectedCandidate")):
            raise AssertionError("evaluation feature leaked labels")
        ...
        locked=S12.predict(downstream,observation,set(S12.VARIANTS[-1][2]))
    ...
    artifact={
        "schemaVersion":"s13-policy-eval-predictions-v1",
    ...
    print(f"SPRINT13_POLICY_EVAL_PREDICTION PASS cases={len(rows)} decisive_policy={sum(1 for r in rows if r['policyDecision']!='UNKNOWN')}")
