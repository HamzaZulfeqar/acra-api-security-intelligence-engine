...
echo "ACRA SPRINT 11 STANDALONE FINAL VERIFICATION"
...
bash ./scripts/build-sprint11-distribution.sh
...
unzip -q dist/acra-sprint11-bundle.zip -d "$EXTRACT"
(
  cd "$EXTRACT/acra-sprint11"
  sha256sum -c SHA256SUMS.txt
  bash -n acra-standalone.sh
)

HOME_DIR="$EXTRACT/home"
java -jar "$EXTRACT/acra-sprint11/acra-standalone.jar" \
  --port=18821 \
  --home="$HOME_DIR" \
  --no-browser > "$EXTRACT/standalone.log" 2>&1 &
PID=$!
for i in $(seq 1 60); do
  if curl --fail --silent http://127.0.0.1:18821/api/health > "$EXTRACT/health.json"; then
    break
  fi
  sleep 0.2
done
...
curl --fail --silent http://127.0.0.1:18821/ | grep -q 'Standalone Security Workbench'
kill "$PID"
wait "$PID" 2>/dev/null || true
PID=""

bash ./scripts/package-sprint11-standalone.sh

echo "SPRINT11_STANDALONE_FINAL_VERIFICATION PASS"
