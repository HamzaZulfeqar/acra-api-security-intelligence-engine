...
cd "$ROOT"
OUT="$ROOT/build/s7-final-package"
rm -rf "$OUT"
mkdir -p "$OUT"
...
root=Path.cwd()
out=root/"build"/"s7-final-package"
zip_path=out/"acra-sprint-07-final.zip"
manifest_path=out/"sprint7-package-manifest.txt"
verify_path=out/"sprint7-package-verification.json"
sha_path=out/"acra-sprint-07-final.zip.sha256"
...
with zipfile.ZipFile(zip_path) as archive:
    ...
    with tempfile.TemporaryDirectory(prefix="acra-s7-verify-") as temp:
        target=Path(temp)
...
echo "SPRINT7_PACKAGE_VERIFICATION PASS"
