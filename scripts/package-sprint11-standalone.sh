...
cd "$ROOT"
OUT="$ROOT/build/s11-final-package"
rm -rf "$OUT"
mkdir -p "$OUT"
...
root=Path.cwd()
out=root/"build"/"s11-final-package"
zip_path=out/"acra-sprint-11-final.zip"
manifest_path=out/"sprint11-package-manifest.txt"
verify_path=out/"sprint11-package-verification.json"
sha_path=out/"acra-sprint-11-final.zip.sha256"
...
with zipfile.ZipFile(zip_path) as archive:
    ...
    with tempfile.TemporaryDirectory(prefix="acra-s11-verify-") as temp:
        target=Path(temp)
...
echo "SPRINT11_PACKAGE_VERIFICATION PASS"
